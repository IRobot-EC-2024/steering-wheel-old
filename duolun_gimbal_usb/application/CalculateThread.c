            ...
            if(CHASSIS_ROTATE_SWITCH_KEYMAP)//小陀螺模式
                Chassis.ChassisState=CHASSIS_ROTATE;
								...
								if(CHASSIS_STOP_KEYMAP)
										chassis_no_follow_flag=(chassis_no_follow_flag+1)%2;
...
void BoomBayCover(void)
{
    ...
    if ((cover_flag == 1)  ||  (cover_flag == 0)) {
        __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_1, 1800);//四号：2245 三号 1250    打开
     }
    if (cover_flag == 2) {
         __HAL_TIM_SetCompare(&htim1, TIM_CHANNEL_1,940);//四号：500   关闭
    }
...
void ShootSpeedAdopt(void)
{
	...
	else if(ammo_speed_r>7800) ammo_speed_l=7800;
