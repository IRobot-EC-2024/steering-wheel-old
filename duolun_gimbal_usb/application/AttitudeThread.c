...
void AttitudeThread(void const *pvParameters) {
  ...
  IMU_QuaternionEKF_Init(10, 0.001, 100000000, 0.9996, 0);
  while (1) {
    AHRS_update(INS_quat, 0.001f, bmi088_real_data.gyro, bmi088_real_data.accel);
    get_angle(QEKF_INS.q, INS_angle + INS_YAW_ADDRESS_OFFSET, INS_angle + INS_PITCH_ADDRESS_OFFSET,
...
const fp32 *get_gyro_data_point(void) { return bmi088_real_data.gyro; }
...
void GetCurrentQuaternion(fp32 q[4]) { memcpy(q, QEKF_INS.q, sizeof(QEKF_INS.q)); }
