 ...
 * @retval         none
 */
void cascade_PID_init(cascade_pid_t *c_pid, const fp32 oPID[3], const fp32 iPID[3], fp32 oPID_max_out,
...
fp32 PD_calc(pid_type_def *pid, fp32 pref, fp32 dref, fp32 pset) {
  if (pid == NULL) {
    return 0.0f;
  }
  pid->error[2] = pid->error[1];
  ...
  pid->set = pset;
  pid->fdb = pref;
  pid->error[0] = pset - pref;
  pid->Pout = pid->Kp * pid->error[0];
  pid->Dout = pid->Kd * (0 - dref);
  //	if(pid->Pout*pid->Pout<pid->Dout*pid->Dout&&pid->Dout*pid->Pout<0)	pid->Dout/=2.0;
  pid->Iout += pid->Ki * pid->error[0];  // pid->Iout = 0;
  LimitMax(pid->Iout, pid->max_iout);
  pid->out = pid->Pout + pid->Iout + pid->Dout;
  LimitMax(pid->out, pid->max_out);
  return pid->out;
